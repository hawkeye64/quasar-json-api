...
const
  ...
  typeRoot = path.resolve(rootDir, './types'),
  distRoot = path.resolve(rootDir, './dist/types'),
...
function writeIndexDTS (apis) {
  ...
  // We expose `ts-helpers` because they are needed
  ...
  if (importName.length > 0) {
    ...
    if (fs.existsSync(tsTypes)) {
      ...
      console.error(`warning: 'types.d.ts' missing for import declarions needed for "${importName.join(', ')}"`)
    }
  }
